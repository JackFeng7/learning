const array = `mirrored beige bags contain 1 drab brown bag, 3 dotted crimson bags.
...
bright red bags contain 1 pale beige bag.`.split(/\n/);
...
function search(tree) {
    tree.parents = findColorAndAdd(tree.color);
...
function findColorAndAdd(color) {
    const parents = [];
    for (let row of array) {
        const currentColor = row.split('contain')[0].trim().split('bags')[0].trim();
        if (row.includes(color) && currentColor !== color && !colors.includes(currentColor)) {
...
function search2(tree) {
    tree.children = findColorAndAdd2(tree.color, tree.count);
    colors2 += tree.color !== 'shiny gold' ?  tree.count : 0;
    sum.push(tree.count);
...
function findColorAndAdd2(color, parentCount) {
    const children = [];
    for(let row of array) {
        const currentColor = row.split('contain')[0].trim().split('bags')[0].trim();
        const currentContain = row.split('contain')[1].trim();
        if (currentColor === color) {
            const containColors = currentContain.split(',');
            for (let containColor of containColors) {
                const containColorCount = isNaN(parseInt(containColor.trim())) ? 0 : parseInt(containColor.trim());
                const currentColor = containColor.split(containColorCount)[1] ? containColor.split(containColorCount)[1].trim().split(containColorCount > 1 ?'bags' : 'bag')[0].trim() : 0;
                children.push({
                    color: currentColor,
                    count: Number(containColorCount) * parentCount,
                    children: [],
                });
            }
        }
