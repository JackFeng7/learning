...
// part2
const strategy2 = {
    byr: (num) => num.length === 4 && Number(num) >= 1920 && Number(num) <= 2002 ,
    iyr: (num) => num.length === 4 && Number(num) >= 2010 && Number(num) <= 2020,
    eyr: (num) => num.length === 4 && Number(num) >= 2020 && Number(num) <= 2030,
    hgt: (str) => str.includes('cm') ? (parseInt(str) >= 150 &&  parseInt(str) <= 193) : (str.includes('in') ? (parseInt(str) >= 59 &&  parseInt(str) <= 76) : false),
    hcl: (str) => new RegExp(/^#([0-9]|[a-f]){6}/).test(str),
    ecl: (str) => ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl','oth'].includes(str),
    pid: (str) => str.length === 9 && new RegExp(/([0-9]){9}/).test(str),
    cid: () => true,
};

// 查询入口
function search(arr, strategies) {
    let length = 0;
    for(let i = 0; i < arr.length; i++) {
        const currentPerson = arr[i].split(/[' ', \n]/);
        if (currentPerson.length === Object.keys(strategies).length) {
            length = checkStrategy(strategies, currentPerson) ? (length + 1) : length;
        } else if (currentPerson.length === (Object.keys(strategies).length - 1) && currentPerson.findIndex((item) => item.includes('cid')) < 0) {
            length = checkStrategy(strategies, currentPerson) ? (length + 1) : length;
        }
    }
    return length;
...
// 判断策略
function checkStrategy(strategies, infos) {
    let flag = true;
    for(let currentFieldInfo of infos) {
        const infoArr = currentFieldInfo.split(':');
        const checkCurrentStrategyRes = strategies[infoArr[0]](infoArr[1]);
        if (!checkCurrentStrategyRes) {
            flag = false;
            break;
        }
    }
...
console.log(search(array, strategy1));

console.log(search(array, strategy2));