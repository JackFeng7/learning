...
// part1
function part1(allGroup) {
    let length = 0;
    Object.values(allGroup).forEach((singleGroup) => {
        length += Object.values(singleGroup.data).length;
    });
    return length;
}

// part2
function part2(allGroup) {
    let length = 0;
    Object.values(allGroup).forEach((singleGroup) => {
        length += Object.values(singleGroup.data).filter((item) => item === singleGroup.personCount).length;
    });
    return length;
}
...
function search(array, fn) {
    const allGroup = {};
    array.forEach((singleGroupStr, index) => {
        const singleGroupArray = singleGroupStr.split(/\n/);
        const data = singleGroupArray.join('').split('').reduce((p, k) => (p[k]++ || (p[k] = 1), p), {});
        allGroup[index] = {
            personCount: singleGroupArray.length,
            data,
        };  
    });
    return fn(allGroup);
...
console.log(search(array, part1), search(array,part2));